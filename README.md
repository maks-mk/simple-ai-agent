...
**v7.2b (Refined UI & Centralized Theme)**
...
- **Robust Process Management:** Инструменты управления процессами поддерживают "Kill Tree" (рекурсивное завершение), идемпотентны (не падают, если процесс уже мертв) и имеют кросс-платформенные фоллбеки (psutil -> taskkill/kill -> pkill).
...
| Файл | Назначение |
| :--- | :--- |
| `agent.py` | **Core Flow.** Инициализация и сборка графа выполнения. |
| `agent_cli.py` | **UI.** Точка входа, обработка ввода/вывода, спиннеры. |

### Модуль `core`
| Файл | Назначение |
| :--- | :--- |
| `nodes.py` | **Node Logic.** Реализация узлов графа (Agent, Tools, Summarize, Filter). |
| `constants.py` | **Constants.** Централизованное хранение промптов, списков стоп-слов и триггеров. |
| `config.py` | **Settings.** Pydantic-конфигурация, загрузка переменных окружения. |
| `state.py` | **State.** Определение структуры состояния (TypedDict). |
| `safety_guard.py` | **Security.** Политики безопасности и проверки перед записью файлов. |
